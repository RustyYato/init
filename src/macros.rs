...
macro_rules! init_struct {
    ($struct_name:path { $($field:ident:$field_value:expr),+ $(,)? }) => {
        $crate::try_from_fn(|mut uninit| {
            ...
            // SAFETY: this pointer deref doesn't actually happen
            unsafe { let $struct_name { $($field: _,)* } = *ptr; }
            $(
...
impl GetConverter<core::convert::Infallible, ()> for &&&Wrapper<'_, core::convert::Infallible> {
...
impl<T> GetConverter<T, T> for &&Wrapper<'_, T> {
    type Converter = TagFrom<T, T>;
